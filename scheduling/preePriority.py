...
class PreemptivePriority:
        ...
        if page_no == "1":
            ...
            if self.grant_chart != []:
                if next_running_p == running_p:
                    ...
                    st.markdown(f"**Grant chart after prioritizing P{next_running_p.pid}({next_running_p.bt}, {next_running_p.priority})**")
    ...
    def simulate_pp(self, page_no="1"):
        ...
        while True:
            ...
            if self.current_time == 0:
                #get response time:
                if running_p.rt == 0 and f'P{running_p}({running_p.bt}, {running_p.priority})'not in self.grant_chart:
                ...
                #run the process:
                self.grant_chart.append(f'P{running_p.pid}({running_p.bt+1}, {running_p.priority})')
